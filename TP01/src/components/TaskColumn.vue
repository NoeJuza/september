<template>
    <v-col  cols="12" md="4" class="pa-0">
        <v-container @click="toggleCollapsed('show'+props.name)" :class="{ 'clickable': isSmallScreen }" class="d-flex justify-center align-center bg-surface pa-0 pa-md-2 rounded-lg">
            <h1>{{props.name}}</h1>
        </v-container>
        <div v-if="notCollapsed">
            <v-container class="align-center justify-center d-flex">
                <v-btn @click="btnHandler()" icon="mdi-plus" class="justify-center bg-accentuated-surface rounded-lg"></v-btn>
            </v-container>
